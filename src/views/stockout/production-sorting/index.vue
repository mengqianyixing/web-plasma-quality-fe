  ...
  <PageWrapper contentFullHeight >
    ...
    <div class="card-bar mt-1 mb-1 bg-[#fff] h-73.5" >
      ...
      <div class="card-bar-body flex w-100% p-3 bg-[#fff] overflow-x-auto" ref="topBoxBarRef">
        <Card v-for="item in topBoxData" :key="item.title" size="small" :id="item.immTypeName" class="one-card min-w-57.5 mr-3 flex-shrink-0 h-57.5" :class="{selected: item.isSelected}" :title="`${item.title}(${item.sortCount}/${item.totalCount})`">
          <template #extra><a class="mr-1" @click="_sortingMouldAssembling(item)">合箱</a><a @click="_sortingBoxSealing(item)">封箱</a></template>
          <div class="text-4" v-for="one in item.bagNos" >{{ one }}</div>
        </Card>
      </div>
    </div>
    ...
    <div class="card-bar mt-1 mb-1 bg-[#fff] h-73.5" >
      ...
      <div class="card-bar-body flex w-100% p-3 bg-[#fff] overflow-x-auto" ref="bottomBoxBarRef">
        <Card v-for="item in bottomBoxData" :key="item.title" size="small" :id="item.immTypeName" class="one-card min-w-57.5 mr-3 flex-shrink-0 h-57.5" :class="{selected: item.isSelected}" :title="`${item.title}(${item.sortCount}/${item.totalCount})`">
          <template #extra><a class="mr-1" @click="_sortingMouldAssembling(item)">合箱</a><a @click="_sortingBoxSealing(item)">封箱</a></template>
          <div class="text-4" v-for="one in item.bagNos" >{{ one }}</div>
        </Card>
      </div>
...
<script lang="tsx" setup>
  ...
  const prepareSchema: DescItem[] = [
    ...
    },
    {
      ...
      render() {
        return (
          <div>
            {prepareData.value.prepareState ? prepareStateMap.get(prepareData.value.prepareState as prepareStateValueEnum) : ''}
          </div>
        );
      }
    },
    ...
    },
    {
      ...
      render() {
        return (
          <div>
            {prepareData.value.sortTotal ?  `${prepareData.value.sortCount}/${prepareData.value.sortTotal}` : ''}
          </div>
        );
      }
    },
    {
      ...
      render() {
        return (
          <div>
            {prepareData.value.proTotal ?  `${prepareData.value.proSortCount}/${prepareData.value.proTotal}` : ''}
          </div>
        );
      }
    },
  ...
  const batchSchema: DescItem[] = [
    ...
    },
    {
      ...
      render() {
        return (
          <div>
            {batchData.value?.batchSummary?.sortTotal ?  `${batchData.value.batchSummary.sortCount}/${batchData.value.batchSummary.sortTotal}` : ''}
          </div>
        );
      }
    },
    {
      ...
      render() {
        return (
          <div>
            {batchData.value.pros?.totalCount ?  `${batchData.value.pros?.sortCount}/${batchData.value.pros?.totalCount}` : ''}
          </div>
        );
      }
    },
  ...
  async function handlePressEnter(e) {
    if (e.code === 'Enter' || e.code === 'NumpadEnter') {
      ...
      batchData.value = {batchSummary: data.batchSummary, pros: data.pros, unPro: data.unPro, utrkUnPro: data.utrkUnPro};
      ...
      if(data.pros?.bagNos?.length) { // 可投产血浆列表有长度，说明正在挑的是可投产的，更新血浆列表
      ...
      if(data?.fullBox === true) {
        Modal.confirm({
          ...
          content: createVNode('div', { style: 'color:red;' }, '该箱已满请进行封箱操作，要打印箱签吗?'),
          onOk() {
            console.log('OK');
          },
          onCancel() {
        ...
        schema: [... batchSchema, 
          {
            ...
            render() {
              return (
                <div>
                  {batchData.value.utrkUnPro?.total ?  `${batchData.value.utrkUnPro?.sortCount}/${batchData.value.utrkUnPro?.total}` : ''}
                </div>
              );
            }
          },
          ...
          schema: [... batchSchema, 
              {
                ...
                render() {
                  return (
                    <div>
                     {batchData.value.unPro?.total ?  `${batchData.value.unPro?.sortCount}/${batchData.value.unPro?.total}` : ''}
                    </div>
                  );
                }
              },
